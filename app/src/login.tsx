...
interface Props {
  setCurrentPage: (
    page: 'home' | 'teacher' | 'personal' | 'login' | 'signup' | 'dashboard' | 'createclass' | 'classroom' | 'classmanagement'
  ) => void;
}
...
const LoginPage: React.FC<Props> = ({ setCurrentPage }) => {
  ...
  const slideAnim = useRef(new Animated.Value(1000)).current; // Start position off-screen (below the viewport)

  useEffect(() => {
    // Slide-up animation when the component mounts
    Animated.timing(slideAnim, {
      toValue: 200, // End position (fully visible)
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      if (error) {
        Alert.alert('Login Failed', 'Invalid credentials or not a teacher account!');
      } else if (data) {
        await AsyncStorage.setItem('userSession', JSON.stringify(data)); // Save full user session
        await AsyncStorage.setItem('user', data.username); // Save the username separately
